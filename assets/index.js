...
function simulateUserInteraction() {
  setTimeout(() => document.querySelector(".tree").click(), DELAY_TIME_MS * 5)
}
...
function addOrnamentalsOnLeaf(treeLeafElement, width) {
  ...
  for(let i = 0; i < amountOfOrnamentals; i++) {
    ...
    treeOrnamentalElement.style.transform = `translate(${0}px, ${getRandomNumber(-5, 5)}px)`
...
function getRandomOrnamental() {
  const randomOrnamental = ORNAMENTALS[getRandomNumber(0, ORNAMENTALS.length - 1)]
...
async function bubbleSort(delayMs = DELAY_TIME_MS) {
  ...
  treePartContainers = Array.from(treePartContainers).slice(0, treePartContainers.length - 1)
  ...
  for (let i = 0; i < treePartContainers.length; i += 1) {
    for (let j = 0; j < treePartContainers.length - i - 1; j += 1) {
      ...
      treePartContainers[j].firstChild.style.backgroundColor = COLOR_LIGTH_GREEN

      // To wait for 0.5s
      await delay(delayMs)
      const width1 = parseFloat(treePartContainers[j].getAttribute("data-width"))
      const width2 = parseFloat(treePartContainers[j + 1].getAttribute("data-width"))
    ...
    treePartContainers[treePartContainers.length - i - 1].firstChild.style.backgroundColor = COLOR_DARK_GREEN
  }
}
